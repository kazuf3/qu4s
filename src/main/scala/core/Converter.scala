...
/** OpenQASM converter object is to transpile Qu4s Circuit into OpenQASM 2.0 as String. In order to run OpenQASM2.0 on
  * IBM Quantum, it uses IBM Quantum default library "qelib1.inc".
  * ==Usage==
  *
  * {{{
  * implicit val c = new Circuit
...
"""
    ...
    head ++ {
      qubits ++ constants ++ Seq("") ++ c.gates.map(_.toOpenQASM()) ++ Seq("") ++ measure
    }.mkString("\n")
  }
  implicit class GateWithOpenQASM(g: Gate) {
